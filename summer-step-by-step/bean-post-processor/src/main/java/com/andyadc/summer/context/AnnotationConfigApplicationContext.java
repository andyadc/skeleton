...
public class AnnotationConfigApplicationContext {
    ...
    void tryInjectProperties(BeanDefinition def, Class<?> clazz, Object bean, AccessibleObject acc) throws ReflectiveOperationException {
        ...
        if (value != null) {
            ...
            if (field != null) {
                logger.info("@Value Field injection: {}.{} = {}", def.getBeanClass().getName(), accessibleName, propValue);
                field.set(bean, propValue);
            }
            if (method != null) {
                logger.info("@Value Method injection: {}.{} ({})", def.getBeanClass().getName(), accessibleName, propValue);
        ...
        if (autowired != null) {
            ...
            if (depends != null) {
                if (field != null) {
                    logger.info("@Autowired Field injection: {}.{} = {}", def.getBeanClass().getName(), accessibleName, depends);
                    field.set(bean, depends);
                }
                if (method != null) {
                    logger.info("@Autowired Method injection: {}.{} ({})", def.getBeanClass().getName(), accessibleName, depends);
    ...
    void checkFieldOrMethod(Member member) {
        ...
        if (Modifier.isFinal(mod)) {
            ...
            if (member instanceof Method method) {
                logger.warn(
                        "Inject final method should be careful because it is not called on target bean when bean is proxied and may cause NullPointerException. method: {}", method);
            }
        }
    }
    ...
    private Object getProxiedInstance(BeanDefinition def) {
        ...
        for (BeanPostProcessor beanPostProcessor : reversedBeanPostProcessors) {
            ...
            if (restoredInstance != beanInstance) {
                logger.info("BeanPostProcessor {} specified injection from {} to {}.", beanPostProcessor.getClass().getSimpleName(),
